...
from integrated_qa_system.rag_qa.core.document_processor import process_documents  # 导入处理文档的函数
...
# 获取数据目录
this_file_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data")
...
def main(query_mode=True, directory_path="data"):
    ...
    def call_dashscope(prompt):
        ...
        try:
            ...
            logger.error(f"LLM API (call_dashscope) 调用失败: {e1}")
            return f"错误: 调用LLM失败 - {e1}"
...
if __name__ == "__main__":
    ...
    main(query_mode=True, directory_path=args.data_dir)